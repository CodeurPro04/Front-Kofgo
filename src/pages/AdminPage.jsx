...
import { Document, Packer, Paragraph, Table, TableCell, TableRow, WidthType, TextRun, HeadingLevel, BorderStyle, AlignmentType, ShadingType } from "docx";
...
export default function AdminDashboard() {
  ...
  const exportToExcelAdvanced = () => {
    ...
    const data = filteredProjects.map((p) => ({
      ...
      "Date de création": p.created_at,
      Statut: "Reception",
    }));

    // 2️⃣ Créer une feuille principale "Projets"
    const worksheet = XLSX.utils.json_to_sheet(data);

    // Ajuster largeur des colonnes
    const maxLengths = data.map((row) =>
      Object.values(row).map((v) => v?.toString().length || 10)
    );
    const colWidths = [];
    Object.keys(data[0]).forEach((_, idx) => {
      colWidths.push({ wch: Math.max(...maxLengths.map((r) => r[idx])) + 5 });
    });
    worksheet["!cols"] = colWidths;

    // 3️⃣ Créer une feuille "KPI"
    const statusCounts = filteredProjects.reduce((acc, proj) => {
      const status = "Reception"; // remplace par proj.status si tu as
      acc[status] = (acc[status] || 0) + 1;
      return acc;
    }, {});

    const kpiData = [
      ["Statut", "Nombre de projets"],
      ...Object.entries(statusCounts),
    ];
    const kpiSheet = XLSX.utils.aoa_to_sheet(kpiData);

    // 4️⃣ Créer le classeur et ajouter les feuilles
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Projets");
    XLSX.utils.book_append_sheet(workbook, kpiSheet, "KPI");

    // 5️⃣ Générer le fichier Excel
    const excelBuffer = XLSX.write(workbook, {
      bookType: "xlsx",
      type: "array",
    });
    const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
    saveAs(blob, "Projets_Avance.xlsx");
  };

  const exportToWord = () => {
    // Créer les lignes du tableau Word
    const rows = [
      new TableRow({
        children: [
          new TableCell({ children: [new Paragraph("Client")] }),
          new TableCell({ children: [new Paragraph("Email")] }),
          new TableCell({ children: [new Paragraph("Projet")] }),
          new TableCell({ children: [new Paragraph("Description")] }),
          new TableCell({ children: [new Paragraph("Date de création")] }),
      ...
      ...filteredProjects.map(
        (project) =>
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph(project.fullname)] }),
              new TableCell({ children: [new Paragraph(project.email)] }),
              new TableCell({ children: [new Paragraph(project.objectif)] }),
              new TableCell({ children: [new Paragraph(project.description)] }),
              new TableCell({ children: [new Paragraph(project.created_at)] }),
              new TableCell({ children: [new Paragraph("Reception")] }),
            ],
          })
      ),
    ];
      ...
      sections: [
        {
          children: [
            new Paragraph({
              text: "Liste des Projets Clients",
              heading: "Heading1",
            }),
            new Table({
              rows,
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-200">
              <div className="p-6 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-gray-50 to-white">
                ...
                <div className="flex space-x-3">
                  <button
                    className="px-4 py-2 bg-white border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 flex items-center"
